...
# Extract pronunciation first
pronunciation_match = re.search(r'\[([^\]]+)\]', body)
pronunciation = pronunciation_match.group(1) if pronunciation_match else None

# Extract the word (on the line before pronunciation)
if pronunciation_match:
    word_text = body[:pronunciation_match.start()]
    lines = word_text.split('\n')
    for line in reversed(lines):
        line = line.strip()
        # Word is the first word before any URL or bracket
        if line and not line.startswith('['):
            # Extract just the word (before any URLs or brackets)
            word_match = re.match(r'^([a-z]+)', line, re.IGNORECASE)
            if word_match:
