...
.text as $text |
(
    if (($text | contains("\u0024MOG")) or ($text | contains(" MOG")) or ($text | contains(",MOG")) or ($text | contains(".MOG")) or ($text | test(" MOG$"; "i"))) then "MOG"
    elif (($text | contains("\u0024KEYCAT")) or ($text | contains(" KEYCAT")) or ($text | contains(",KEYCAT")) or ($text | contains(".KEYCAT")) or ($text | test(" KEYCAT$"; "i"))) then "KEYCAT"
    elif (($text | contains("\u0024KTA")) or ($text | contains(" KTA")) or ($text | contains(",KTA")) or ($text | contains(".KTA")) or ($text | test(" KTA$"; "i"))) then "KTA"
    elif (($text | contains("\u0024REI")) or ($text | contains(" REI")) or ($text | contains(",REI")) or ($text | contains(".REI")) or ($text | test(" REI$"; "i"))) then "REI"
    elif (($text | contains("\u0024VIRTUAL")) or ($text | contains(" VIRTUAL")) or ($text | contains(",VIRTUAL")) or ($text | contains(".VIRTUAL")) or ($text | test(" VIRTUAL$"; "i"))) then "VIRTUAL"
    elif (($text | contains("\u0024TIBBIR")) or ($text | contains(" TIBBIR")) or ($text | contains(",TIBBIR")) or ($text | contains(".TIBBIR")) or ($text | test(" TIBBIR$"; "i"))) then "TIBBIR"
    else empty end
) as $token |
...
# Build message
{
    ...
    echo ""
    echo "🐦 *TOP 5 CRYPTO TWEETS* (Last 12h)"
    echo "───────────────────────────────────"
    
    # Get tweets from last 12 hours
    TWEETS=$(bird home --following --count 100 --json 2>/dev/null)
    TWELVE_HOURS_AGO=$(date -d "12 hours ago" +%s 2>/dev/null || date -u -v-12H +%s)
    
    if [ -n "$TWEETS" ]; then
        # FIXED: Use process substitution and a temp file to avoid subshell variable issue
        ...
        while IFS="|" read -r created_at username likes tweet_id text; do
            ...
            if [ -n "$tweet_ts" ] && [ "$tweet_ts" -gt "$TWELVE_HOURS_AGO" ]; then
                echo "• @$username (${likes}❤️): ${text:0:60}..."
    ...
    echo ""
    echo "🎯 *FAVORITE TOKEN TWEETS* (Last 12h, max 5)"
    echo "─────────────────────────────"
    
    if [ -n "$TWEETS" ]; then
        # FIXED: Same approach - avoid subshell
        ...
        while IFS="|" read -r created_at username likes tweet_id token text; do
            ...
            tweet_ts=$(date -d "$created_at" +%s 2>/dev/null)
            if [ -n "$tweet_ts" ] && [ "$tweet_ts" -gt "$TWELVE_HOURS_AGO" ]; then
                echo "• [$token] @$username (${likes}❤️): ${text:0:60}..."
                echo "  https://x.com/$username/status/$tweet_id"
                if echo "$text" | LC_ALL=C grep -qP '[^\x00-\x7F]'; then
                    translation=$(translate_text "$text" 2>/dev/null)
                    if [ -n "$translation" ]; then
